...
from flask import render_template, flash, redirect, url_for, jsonify, send_from_directory
...
from settings import BOT_TOKEN, BASE_URL, RULES, MONTHS, SET_WEBHOOK
...
def index():
    if request.method == 'POST' and request.get_json()['update_id']:
        js_data = request.get_json()
        try:
            chat_id = js_data['callback_query']['message']['chat']['id'] if 'callback_query' in js_data else js_data['message']['chat']['id']
        except KeyError:
            chat_id = None
        try:
            first_name = js_data['callback_query']['message']['chat']['first_name'] if 'callback_query' in js_data else js_data['message']['chat']['first_name']
        except KeyError:
            first_name = None
        try:
            last_name = js_data['callback_query']['message']['chat']['last_name'] if 'callback_query' in js_data else js_data['message']['chat']['last_name']
        except KeyError:
            last_name = None
        try:
            username = js_data['callback_query']['message']['chat']['username'] if 'callback_query' in js_data else js_data['message']['chat']['username']
        except KeyError:
            username = None
        try:
            message_id = js_data['callback_query']['message']['message_id'] if 'callback_query' in js_data else js_data['message']['message_id']
            msg_text = js_data['callback_query']['message']['text'] if 'callback_query' in js_data else js_data['message']['text']
        except KeyError:
            pass
        ...
        # Callback handlers
        if 'callback_query' in js_data:
            data = js_data['callback_query']['data']
            print(data)
            buttons = []
            if re.search(r'(restaurant_[0-9]+$)|'
                ...
                if address:
                    ...
                    bt_text = 'Отправить и узнать время доставки'
                    buttons = [
                        [InlineKeyboardButton(bt_text, callback_data=cb_data1)],
                ...
                if len(cart) == 0:
                    ...
                    try:
                        ...
                        print("Cart item remove error:", e)
                ...
                if not restaurant.min_total or total >= restaurant.min_total:
                    buttons = [
                ...
                try:
                    ...
                    BOT.send_message(chat_id=rest.service_uid, text=text,
                                     reply_markup=InlineKeyboardMarkup(buttons))
                    write_history(message_id, chat_id, text, is_bot=True)
                ...
                order.order_state = 'Заказ принят рестораном, ' + state + time_text[10:]
                ...
                if details:
                    ...
                    try:
                        ...
                        Order.query.filter_by(id=order_id).first().order_state = 'Заказ отменен'
                        db.session.commit()
                ...
                total = sum(list(map(lambda x: x.order_dish_cost * x.order_dish_quantity, details)))
                ...
                if re.search(r'(^order_[0-9]+_user_confirm$)', data):
                    text = f'Заказ отправлен ждите ответа ресторана {rest_name}. За статусом заказа смотрите в "Мои ' \
                           f'заказы" в разделе Справка. '
                    status = 'Заказ отправлен, ожидание ответа ресторана'
                    Order.query.filter_by(id=int(data.split('_')[1])).first().order_state = status
                    BOT.send_message(chat_id=order.uid, text=text)
                ...
                if not check:
                    ...
                    BOT.answer_callback_query(
                        callback_query_id=js_data['callback_query']['id'],
            ...
            try:
                ...
                if parsed == '/start':
                    text = "*Добро пожаловать в Robofood*😊\n_Здесь_ `Вы` можете заказать еду из ресторанов на " \
                           "доставку и самовывоз\.\ Начните с кнопки “Меню” или наберите в сообщении " \
                           "интересующую еду 🍱🥤🍕 "
                    remove_keyboard = {'remove_keyboard': True}
                    remove_keyboard_encoded = json.dumps(remove_keyboard)
                    BOT.send_message(chat_id, text, reply_markup=remove_keyboard_encoded, parse_mode=ParseMode.MARKDOWN_V2)
                elif parsed == '/my_orders':
                    ...
                    if order and rest.filter_by(id=order.order_rest_id).first():
                        ...
                        text = 'У Вас пока нет оформленных заказов'
                    BOT.send_message(chat_id=chat_id, text=text)
                elif parsed in ('Рестораны', '/restaurants'):
                    rest_menu_send_msg(chat_id)
                elif parsed in ('Комбо Наборы (КБ)', '/combo_set'):
                    ...
                    write_history(message_id, chat_id, text, is_bot=True)
                elif parsed in ('Рекомендуем', '/recommend'):
                    ...
                    write_history(message_id, chat_id, text, is_bot=True)
                elif parsed in ('Акции', '/promotions'):
                ...
                elif parsed in ('Корзина', '/show_cart'):
                    cart = db.session.query(Cart).filter_by(user_uid=chat_id).all()
                    if len(cart) == 0:
                        text = 'Ваша корзина пуста'
                        BOT.send_message(chat_id=chat_id, text=text)
                ...
                elif parsed == 'Оформить заказ':
                    BOT.send_message(chat_id, 'Вы выбрали оформить заказ')
                elif parsed == 'Статистика' and chat_id in [113737020, 697637170]:
                    ...
                    try:
                        if re.search(r'^Напишите пожалуйста что хотите изменить в заказе № [0-9]+ .+$', bot_msg):
                            order_id = int(re.search(r'[0-9]+', msg_text).group(0))
                            order = db.session.query(Order).filter_by(id=order_id).first()

                            BOT.send_message(chat_id=order.uid, text=msg_text)
                            ...
                            # bot_msg = History.query.filter_by(message_id=message_id - 2, is_bot=False).first().message_text
                            cur_usr = db.session.query(User).filter_by(uid=chat_id).first()
                            cur_usr.phone = msg_text
                            db.session.commit()
                            text = 'Вы указали:\n'
                        ...
                        msg_list = re.sub("[^\w]", " ", msg_text).split()
                        check = False
                        search_words = SearchWords.query.all()
                        for word in search_words:
                            words = re.sub("[^\w]", " ", word.words).split()
                            for msg in msg_list:
                                if msg.lower() in words:
                                    print(msg)
                                    check = True
                                    search_dishes = SearchDishes.query.filter_by(search_words_id=word.id).all()
                                    for current_id, dish in enumerate(search_dishes, start=1):
                                        cat_id = Category.query.filter_by(name=dish.dish_category).first().id
                                        text = Restaurant.query.filter_by(id=dish.rest_id).first().name + '\n'
                                        text += f'<a href="{Dish.query.filter_by(id=dish.dish_id).first().img_link}">.</a>\n'
                                        text += dish.dish_name + '\n'
                                        text += Dish.query.filter_by(id=dish.dish_id).first().composition + '\n'
                                        text += str(Dish.query.filter_by(id=dish.dish_id).first().cost) + ' р.'
                                        try:
                                            cart_count = Cart.query.filter_by(user_uid=chat_id, dish_id=dish.dish_id).first().quantity
                                        except TypeError:
                                            cart_count = 0
                                        except IndexError:
                                            cart_count = 0
                                            ...
                                            cart_count = 0
                                        cb_data_first = f'restaurant_{dish.rest_id}_cat{cat_id}_dish_{dish.dish_id}'
                                        cb_data_last = f'{chat_id}_{message_id + current_id}'
                                        cb_data = f'fav_{chat_id}_{dish.rest_id}_{dish.dish_id}'
                                        buttons = [[
                                            InlineKeyboardButton('⭐️', callback_data=cb_data),
                                            InlineKeyboardButton('-️',
                                                                 callback_data=f'{cb_data_first}_rem_{cb_data_last}'),
                                        ...
                                        total = 0
                                        cart_items = Cart.query.filter_by(user_uid=chat_id).all()
                                        if cart_items:
                                            for item in cart_items:
                                                total += item.price * item.quantity
                                        buttons.append(
                                            [InlineKeyboardButton('Меню ресторана',
                                                                  callback_data=f'restaurant_{dish.rest_id}_menu')])
                                        buttons.append(
                                            [InlineKeyboardButton(f'В корзину: заказ на сумму {total} р.',
                                                                  callback_data='cart')])
                                        BOT.send_message(
                                            chat_id=chat_id,
                                            text=text,
                ...
                print('msg:', js_data['message']['text'])
        ...
        with Image.open(img_file) as img:
            ...
            for user in users:
                if count == 60:
                    sleep(1)
                try:
                    BOT.sendPhoto(chat_id=user.uid, photo=open(path, 'rb'), caption=request.form.get('msg_txt'))
                    count += 1
                # BOT.sendPhoto(chat_id=122791261, photo=open(path, 'rb'), caption=request.form.get('msg_txt'))
                except Exception as e:
                    print(e)
                    pass
                # flash(e)
...
def admin():
    ...
    if promo_dishes_form.promo_dish_submit.data:
        if promo_dishes_form.validate_on_submit() or promo_dishes_form.is_submitted():
            rest_id = promo_dishes_form.rest_id.data if promo_dishes_form.rest_id.data else request.form['promo_rest_selector']
    ...
    if search_dish_form.search_dish_submit.data:
        if search_dish_form.validate_on_submit() or search_dish_form.is_submitted():
            ...
            dish_id = search_dish_form.dish_id.data if search_dish_form.dish_id.data else request.form[f'search_dish_selector_{search_word_id}']
            dish_name = Dish.query.filter_by(id=dish_id).first().name
            category = Dish.query.filter_by(id=dish_id).first().category
            rest_id = search_dish_form.rest_id.data if search_dish_form.rest_id.data else request.form[f'search_dish_rest_selector_{search_word_id}']
            try:
                db.session.add(
                    SearchDishes(
                        dish_id=dish_id,
    ...
    if category_delete_form.category_delete_submit.data:
        if category_delete_form.validate_on_submit() or category_delete_form.is_submitted():
            restaurant_id = request.form['category_del_rest_selector'] if current_user.ownership == 'all' else category_delete_form.restaurant_id.data
            name = request.form['category_delete_select_field'] if current_user.ownership == 'all' else request.form['category_rest_delete_select_field']
...
def rest_menu_edit_msg(chat_id, message_id):
    ...
    if not markup.inline_keyboard:
        ...
        text = 'Пожалуйста, выберите ресторан:'
        BOT.editMessageText(chat_id=chat_id, text=text, message_id=message_id, reply_markup=markup)
...
def stat2():
    ...
    rest = Restaurant.query
    for data in stat_data:
        if ('Подтверждена' or 'Заказ принят рестораном' in data.order_state) and rest.filter_by(id=data.order_rest_id).first():
